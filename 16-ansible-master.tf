...
resource "null_resource" "ansible-runs" {
  ...
  provisioner "remote-exec" {
    inline                                                  = [
                                                                ...
                                                                "ansible-playbook -i /home/localadmin/ansible/inventory -vvvvvvv /home/localadmin/ansible/palo.yml -e mgmt_ip=${azurerm_public_ip.palo_mgmt_ip.ip_address} -e admin_username=${var.vm_username} -e admin_password=${var.vm_password} -e paloname=${azurerm_virtual_machine.pan_vm.0.name} -e gateway=${local.default_gateway} -e gateway_trust=${local.default_gateway_trust} -e gateway_dmz=${local.default_gateway_dmz} -e gateway_dmz_trust=${local.default_gateway_dmz_trust} -e f5_data_subnet=${local.f5_data_subnet} -e env_subscripton=${var.environment} -e globalscape_pub_key=${var.globalscape_pub_key} -e globalscape_priv_key=${var.globalscape_priv_key} -e prod_cert_pub_key=${var.prod_cert_pub_key} -e prod_cert_priv_key=${var.prod_cert_priv_key}",
                                                            ]
